---
date: "2020-09-12"
description: How have GDP compoents changed over time?
draft: false
keywords: ""
slug: blog6
...
#shortening the names of some of the components of GDP 
gdp_time <- gdp_time  %>% 
  mutate(
...
GDP_Calculated <- GDP_Calculated %>% 
  mutate(net_exports= Exports- Imports)%>%  #creating a new column for net_exports 
  select(-Imports, -Exports) %>% #removing the columns 'Imports' and 'Exports' which are not required in the calculation of GDP
  pivot_longer(
    cols=4:7,
...
Next I will now compare the GDP values that I calculated above with the actual values and compute the percentage difference between the two. 
